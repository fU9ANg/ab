...
unpacket(Buf* pBuf, T& obj, int cnt = 0) {
        ...
        char* buf = (char*)pBuf->ptr() + MSG_HEAD_LEN + sizeof(int) + sizeof(T)*cnt;
        size_t len = ((MSG_HEAD*)pBuf->ptr())->cLen - MSG_HEAD_LEN + sizeof(int) + sizeof(T)*cnt;
        recv_buf.assign(buf, len);
